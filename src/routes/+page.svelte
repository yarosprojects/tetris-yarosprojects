...
<main class="w-full min-h-screen flex flex-col justify-center items-center px-4 md:px-0">
    {#if start}
        <Tetris />
    {:else}
        <div class="flex flex-col w-full justify-center items-center gap-2" id="welcome-box">
            <Welcome />

            <button 
                id="start-button" 
                on:click={() => (start = true)} 
                class="outline-none border-none hover:scale-105 active:scale-80 transition-all group" aria-label="Start gane button"
            >
            ...
            <div class="w-full flex flex-col justify-center items-center py-4">
                <a 
                    href="https://github.com/yarosprojects/tetris" 
                ...
                >
                    <i class="icon-[line-md--github] text-3xl text-white/80 hover:text-white transition-all font-extrabold"></i>
                </a>
            </div>
        </div>
    {/if}
